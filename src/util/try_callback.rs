use std::fmt;
use leptos::{store_value, StoredValue, Callable};

use super::OptionExt;

pub struct TryCallback<In: 'static, Out: 'static = ()> (
...
impl<In: 'static, Out: Default + 'static> Callable<In, Out> for TryCallback<In, Out> {
    fn call(&self, input: In) -> Out {
		self.try_call(input).unwrap_or_default_and_log("callback already disposed")
    }
}
